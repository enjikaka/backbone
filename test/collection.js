(function (QUnit) {
  ...
  QUnit.test('Underscore methods', assert => {
    assert.expect(21);
    assert.equal(col.map(model => {
      return model.get('label');
    }).join(' '), 'a b c d');
    assert.equal(col.some(model => {
      return model.id === 100;
    }), false);
    assert.equal(col.some(model => {
      return model.id === 0;
    }), true);
    assert.equal(col.reduce((m1, m2) => {
      return m1.id > m2.id ? m1 : m2;
    }).id, 3);
    assert.equal(col.reduceRight((m1, m2) => {
      return m1.id > m2.id ? m1 : m2;
    }).id, 3);
    ...
    assert.deepEqual(col.groupBy(model => {
      return model.id;
    })[first.id], [first]);
    assert.deepEqual(col.countBy(model => {
      return model.id;
    }), { 0: 1, 1: 1, 2: 1, 3: 1 });
    assert.deepEqual(col.sortBy(model => {
      return model.id;
    })[0], col.at(3));
